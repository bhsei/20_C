...
def model_run():
    ...
    data_str = str(data)
    try:
        input_data = torch.Tensor(json.loads(data_str))
    except:
    ...
    result = None
    try:
        run_result = model_torch(input_data)
        run_result = numpy.asarray(run_result.data).tolist()
        result = {
            'status': 'success',
            'data': run_result
        }
...
def load_model(model_path):
    ...
    global model_torch
    #"D:\研究生课程\研一下\软工实验\deployment\\torch_model\model.pt"
